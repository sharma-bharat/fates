module EDLoggingMortalityMod
   ...
   !  Yi Xu & M.Huang
   !  Date: 09/2017
   !  Modified for the AgBgW case: Bharat Sharma (sharmabd@ornl.gov)
   !  In this file I exported the Carbon and Nitrogen out of the site
   ! ====================================================================================
   ...
   subroutine logging_litter_fluxes(currentSite, currentPatch, newPatch, patch_site_areadis, bc_in)
      ...
      do el = 1,num_elements
         ...
         do while(associated(currentCohort))
            ...
            ! Bharat >>> Before harvest, testing

            write(fates_log(),*) 'Before harvest!', 'ag_wood: ',ag_wood, 'store_m', store_m, &
            'leaf_m',leaf_m,'repro_m',repro_m, 'fnrt_m',fnrt_m, &
            'donate_frac_area_ratio',donate_m2, &
            'retain_frac_area_ratio', retain_m2, &
            ...
            ! bharat: if not c12 and N (exporting out of site)>>>:
	         do c = 1,ncwd-1
               if( element_id .eq. carbon12_element .or. element_id .eq. nitrogen_element) then
                  new_litt%ag_cwd(c)     = new_litt%ag_cwd(c)
                  cur_litt%ag_cwd(c)     = cur_litt%ag_cwd(c)
               else
                  new_litt%ag_cwd(c)     = new_litt%ag_cwd(c) + &
                     ag_wood * SF_val_CWD_frac_adj(c) * donate_m2 ! bharat: earlier it was 'SF_val_CWD_frac(c)* donate_frac_area_ratio'
                  cur_litt%ag_cwd(c)     = cur_litt%ag_cwd(c) + &
                     ag_wood * SF_val_CWD_frac_adj(c) * retain_m2 ! bharat: earlier it was 'SF_val_CWD_frac(c) * retain_frac_area_ratio'
               end if
               ...
               do ilyr = 1,nlevsoil
                  if( element_id .eq. carbon12_element .or. element_id .eq. nitrogen_element) then
                     ...
                     new_litt%bg_cwd(c,ilyr) = new_litt%bg_cwd(c,ilyr) + &
                        ...
                        SF_val_CWD_frac_adj(c) * donate_m2  ! bharat: earlier it was 'SF_val_CWD_frac(c) * donate_frac_area_ratio'

                     cur_litt%bg_cwd(c,ilyr) = cur_litt%bg_cwd(c,ilyr) + &
                        bg_wood * currentSite%rootfrac_scr(ilyr) * &
                        SF_val_CWD_frac_adj(c) * retain_m2 ! bharat: earlier it was 'SF_val_CWD_frac(c) * retain_frac_area_ratio'
                  end if
               end do
            ...
            if( element_id .eq. carbon12_element .or. element_id .eq. nitrogen_element) then
               ...
               new_litt%ag_cwd(ncwd) = new_litt%ag_cwd(ncwd) + ag_wood * &
                  SF_val_CWD_frac_adj(ncwd) * donate_m2   ! bharat: earlier 'SF_val_CWD_frac(ncwd) * donate_frac_area_ratio'
               cur_litt%ag_cwd(ncwd) = cur_litt%ag_cwd(ncwd) + ag_wood * &
                  SF_val_CWD_frac_adj(ncwd) * retain_m2   ! bharat: earlier `SF_val_CWD_frac(ncwd) * retain_frac_area_ratio`
            end if
            ...
            ! bharat >>> addding write commands to see what are the values of dif carbon pools

            write(fates_log(),*) 'afterharvest: ', 'ag_wood: ',ag_wood, 'store_m', store_m, &
            'leaf_m',leaf_m,'repro_m',repro_m, 'fnrt_m',fnrt_m, &
            'donate_frac_area_ratio',donate_m2, &
            'retain_frac_area_ratio', retain_m2, &
            'EL:', el
            ...
            ! ----------------------------------------------------------------------------------------
            bg_wood = direct_dead * (struct_m + sapw_m ) * SF_val_CWD_frac_adj(ncwd) * &   ! bharat earlier: `SF_val_CWD_frac(ncwd)`
                  (1._r8 - prt_params%allom_agb_frac(currentCohort%pft))
            ...
            do ilyr = 1,nlevsoil
               if( element_id .eq. carbon12_element .or. element_id .eq. nitrogen_element) then
                  ...
                  new_litt%bg_cwd(ncwd,ilyr) = new_litt%bg_cwd(ncwd,ilyr) + &
                      bg_wood * currentSite%rootfrac_scr(ilyr) * &
                      donate_m2  ! bharat earlier: `donate_frac_area_ratio`
                  cur_litt%bg_cwd(ncwd,ilyr) = cur_litt%bg_cwd(ncwd,ilyr) + &
                      bg_wood * currentSite%rootfrac_scr(ilyr) * &
                      retain_m2  ! bharat earlier: `retain_frac_area_ratio`
               end if
            end do
            ! <<< c12
            ...
            ! bharat: Allocation store_m to ag_wood (carbon 12 element)>>>
            if( element_id .eq. carbon12_element .or. element_id .eq. nitrogen_element) then
                !ag_wood = ag_wood + (direct_dead+indirect_dead)*(store_m)

                !both leaf_m and store_m are transfered to ag_wood
                !ag_wood = ag_wood + (direct_dead+indirect_dead)*(store_m+leaf_m)

                !all four elemetns were added to ag_wood
                ag_wood = ag_wood + (direct_dead+indirect_dead)* (store_m+leaf_m+repro_m+fnrt_m)
            ...
            ! bharat: <<<
            write(fates_log(),*) 'After afterharvest: ', 'ag_wood: ',ag_wood, 'store_m', store_m, &
            'leaf_m',leaf_m,'repro_m',repro_m, 'fnrt_m',fnrt_m, &
            'donate_frac_area_ratio',donate_m2, &
            'retain_frac_area_ratio', retain_m2, &
            'EL:', el
